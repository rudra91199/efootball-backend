...
const getSingleTournamentFromDB = async (tournamentId) => {
  // 1. Fetch the basic tournament first to check participantType
  const tournament = await Tournament.findById(tournamentId).lean();

  if (!tournament) return null;

  const isTeamType = tournament.participantType === "Team";

  // 2. Build population array dynamically
  const populations = [
    {
      path: "phases.matches",
      populate: [
  ...
  if (isTeamType) {
    ...
    populations.push({ path: "teams" });
  }

  // 4. Execute the population on the document
  return await Tournament.populate(tournament, populations);
};
