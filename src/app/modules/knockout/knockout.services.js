...
const generateGauntletKnockout = async (tournamentId, mainLeagueChampionIds) => {
  try {
    // --- 1. Validation ---
    ...
    if (leaderboard.length < 4) {
        throw new Error("Proving Grounds league is not complete or has fewer than 4 players.");
    }
    const provingGroundsContenders = leaderboard.slice(0, 4).map(p => p.playerInfo);
    ...
    const participantIds = [seed1, seed2, seed3, seed4, seed5, seed6, seed7, seed8];
    ...
    const quarterFinalMatchesToCreate = [
      { knockout: newKnockout._id, round: "Quarter-Final", team1: seed1, team2: seed8, status: "Unpublished" },
      { knockout: newKnockout._id, round: "Quarter-Final", team1: seed4, team2: seed5, status: "Unpublished" },
      { knockout: newKnockout._id, round: "Quarter-Final", team1: seed2, team2: seed7, status: "Unpublished" },
      { knockout: newKnockout._id, round: "Quarter-Final", team1: seed3, team2: seed6, status: "Unpublished" }
    ];
    ...
    for (const match of createdMatches) {
      historyLogsToCreate.push({
        player: match.team1, opponent: match.team2, match: match._id, tournament: tournamentId, result: "Pending",
      });
      historyLogsToCreate.push({
        player: match.team2, opponent: match.team1, match: match._id, tournament: tournamentId, result: "Pending",
      });
    }
